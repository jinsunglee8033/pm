...
@section('content')
    ...
    <form id="frm_search" class="form-horizontal" method="post" action="/admin">
        {{ csrf_field() }}
        <input type="hidden" name="today" id="today"/>
    </form>
    @if (Auth::check() && Auth::user()->account_type == 'L')
        <button type="button" class="btn btn-primary btn-sm" id="btn_search" onclick="normal()">
            Standard
        </button>
        <button type="button" class="btn btn-primary btn-sm" id="btn_search" onclick="today()">
            Today
        </button>
    @endif
