...
//        if (empty($tid)) {
//            return redirect('/sub-agent/error')->with([
//                'error_msg' => 'Your account is not authorized to do ATT Mobile activation. (no tid)
//                \n Please contact your distributor'
//            ]);
//        }
        ...
        return view('sub-agent.activate.att')->with([
    ...
    public function sim(Request $request) {
        try {
            ...
            $sim_obj = StockSim::where('sim_serial', $request->sim)->where('sim_group', 'ATT')->first();
...
//                }
    ...
    public function commission(Request $request) {
        try {
            ...
            $sim_obj = StockSim::where('sim_serial', $request->sim)->where('sim_group', $sim_group)->first();
            $esn_obj = StockESN::where('esn', $request->imei)->where('product', $product_id)->first();

            if (!empty($denom)) {
                $account = Account::find(Auth::user()->account_id);

                $ret_spiff = SpiffProcessor::get_account_spiff_amt($account, $denom->product_id, $denom->denom, 1, 1, null, $request->sim, $request->imei, $sim_group);
    ...
    public function post(Request $request) {
        try {
            ...
            $sim_obj = StockSim::where('sim_serial', $request->sim)->where('sim_group', 'ATT')->where('status', 'A')->first();
            if (empty($sim_obj)) {
                return response()->json([
                    'code' => '-2',
                    'data' => [
            ...
            $discount_obj   = PaymentProcessor::get_rtr_discount_amount($account, 'WATTR', $denom->denom, $rtr_month - 1);
