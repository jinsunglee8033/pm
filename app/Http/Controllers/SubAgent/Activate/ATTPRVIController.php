...
//        }
        ...
        return view('sub-agent.activate.attprvi')->with([
...
//                }
    ...
    public function esn(Request $request) {
        try {
            ...
            if (!empty($esn_mappings) && $esn_mappings > 0) {
                $mapping = StockMapping::where('product', $product_id)->where('sim', $request->sim)->where('esn', $request->esn)->where('status', 'A')->first();
    ...
    public function post(Request $request) {
        try {
            ...
            if (empty($tid)) {
                return redirect('/sub-agent/error')->with([
                  'error_msg' => 'Your account is not authorized to do ATTPRVI Mobile activation. Please contact your distributor'
                ]);
            }
            ...
            $discount_obj   = PaymentProcessor::get_rtr_discount_amount($account, 'WATTPVR', $denom->denom, $rtr_month - 1);
...
//            }
            ...
            if ($sim_type == 'C') {
                ...
                if (!empty($ret['error_msg'])) {
                    ...
                    Helper::send_mail('it@perfectmobileinc.com', '[PM][ATTPRVI][' . getenv('APP_ENV') . '] Failed to charge for consignment SIM / ESN', $msg);
                }
            }
            ...
            if ($request->is_port_in != 'Y') {
                ...
                if (!empty($ret['error_msg'])) {
                    ...
                    Helper::send_mail('it@perfectmobileinc.com', '[PM][ATTPRVI][' . getenv('APP_ENV') . '] Failed to give spiff', $msg);
                }
                ...
                if (!empty($trans->esn)) {
                    ...
                    if (!empty($ret['error_msg'])) {
                        ...
                        Helper::send_mail('it@perfectmobileinc.com', '[PM][ATTPRVI][' . getenv('APP_ENV') . '] Failed to give rebate', $msg);
                    }
                }
                ...
                if (!empty($ret)) {
                    Helper::send_mail('it@perfectmobileinc.com', '[PM][ATTPRVI][' . getenv('APP_ENV') . '] applyRTR issue', $ret);
                }
                ...
                if ($trans->rtr_month > 1) {
                    ...
                    if (!empty($error_msg)) {
                        ...
                        $msg .= ' - error : ' . $error_msg;
                        Helper::send_mail('it@perfectmobileinc.com', '[PM][' . getenv('APP_ENV') . '] ATTPRVI Activation - applyRTR remaining month failed', $msg);
                    }
                }
            }
