...
//        ]);
        ...
        if ($account->act_lyca != 'Y') {
            return redirect('/sub-agent/error')->with([
              'error_msg' => 'Your account is not authorized to do Lyca Mobile activation. Please contact your distributor'
            ]);
        }

        $ret = Helper::check_parents_product($account->id, 'WLYCA');
        if($ret != 'Y') {
            return redirect('/sub-agent/error')->with([
                'error_msg' => 'Your account is not authorized to do Lyca Mobile activation. Please contact your distributor'
            ]);
        ...
        return view('sub-agent.activate.lyca')->with([
    ...
    public function sim(Request $request) {
        try {
            $v = Validator::make($request->all(), [
              'sim' => 'required|regex:/^\d{19}$/'
            ], [
              'sim.regex' => 'The SIM format is invalid or (less than 19 digits)',
            ]);
...
//                    $sim_obj = StockSim::upload_byos($request->sim, null, $product_id, 'Lyca');
//                } else {
//                    return response()->json([
//                      'code' => '-2',
...
//                }
                return response()->json([
                    'code' => '-2',
                    'msg' => 'SIM is not in system, Not allow BYOS'
                ]);
            } else {
                if ($sim_obj->status !== 'A') {
                    return response()->json([
            ...
            $allowed_months = Helper::get_min_month($sim_obj, $account, 'lyca_min_month');
    ...
    public function commission(Request $request) {
        try {
            ...
            if (!empty($denom)) {
                ...
                $ret_spiff = SpiffProcessor::get_account_spiff_amt($account, $denom->product_id, $denom->denom, 1, 1, null, $request->sim, null);
                $spiff = $ret_spiff['spiff_amt'];

                if ($account->rebates_eligibility == 'Y') {
                    $rebate_type = empty($esn_obj) ? 'B' : 'R';
                    $ret_rebate = RebateProcessor::get_account_rebate_amt($rebate_type, $account, $denom->product_id, $denom->denom, 1, 1, null);
...
//            }
            ...
            $sim_obj = StockSim::where('sim_serial', $request->sim)->where('product', $product_id)->where('status', 'A')->first();
            if (empty($sim_obj)) {
                return response()->json([
                  'code' => '-2',
                  'data' => [
...
//            }
            ...
            $discount_obj   = PaymentProcessor::get_rtr_discount_amount($account, 'WLYCA', $denom->denom, $rtr_month - 1);
            ...
            ### Need to add activation fee for Lyca (By Product setting) ###
            $activation_fee = $vendor_denom->fee;
            $fee = $activation_fee + $fee;
...
//            }
            ...
            if ($request->is_port_in != 'Y') {
                // LycaActivationRTR($trans_id, $product_id, $sim, $zip, $amount)
                //$ret = emida2::LycaActivationRTR($trans->id, $vendor_denom->act_pid, $trans->sim, $trans->zip, $trans->denom);
                $ret = emida2::LycaActivationRTR($trans->id, $vendor_denom->act_pid, $trans->sim, $trans->zip, $trans->denom);
            } else {

                // PortIn Eligibility check
//                $ret = emida::LycaCheckPortInEligibility($vendor_denom->act_pid, $o->phone, $o->sim);
//
//                if(!empty($ret['res_code'])) { // response failed -> still pending
//
//                    $o->portstatus = $ret['res_code'];
//                    $o->portable_reason = $ret['res_msg'];
//                    $o->save();
//
//                    Helper::send_mail('it@perfectmobileinc.com', '[Lyca][' . getenv('APP_ENV') . '] Lyca Port-In Eligibility Check Failure', var_export($ret, true));
//                    return response()->json([
//                        'code' => '-7',
//                        'data' => [
//                            'fld'   => 'exception',
...
//                }
//
//                $lyca_error_code = $ret['$lyca_error_code'];
//                $lyca_error_desc = $ret['$lyca_error_desc'];
//
//                if($lyca_error_code != '0') { // Not Okay with PortIn Eligibility
//
//                    return response()->json([
//                        'code' => '-3',
//                        'data' => [
...
//                        ]
//                    ]);
//                }else{ // Okay with Lyca PortIn Eligibility
////                  LycaPortinRTR($trans_id, $product_id, $sim, $mdn, $account_no, $account_psw, $zip, $amount)
//                    $ret = emida::LycaPortinRTR($trans->id, $vendor_denom->act_pid, $trans->sim, $trans->phone, $trans->account_no, $trans->account_pin, $trans->zip, $trans->denom);
//
//                }

                //$ret = emida2::LycaPortinRTR($trans->id, $vendor_denom->act_pid, $trans->sim, $trans->phone, $trans->account_no, $trans->account_pin, $trans->zip, $trans->denom);
                $ret = emida2::LycaPortinRTR($trans->id, $vendor_denom->act_pid, $trans->sim, $trans->phone, $trans->account_no, $trans->account_pin, $trans->zip, $trans->denom);
            }
            ...
            $trans->vendor_tx_id = $request->is_port_in == 'Y' ? $ret['ref_number'] : '';
            $trans->mdate   = Carbon::now();
            $trans->modified_by = $user->user_id;
            $trans->api = 'Y';
            $trans->save();
            ...
            if ($request->is_port_in != 'Y') {
                ...
                if (!empty($ret['error_msg'])) {
                    ...
                    Helper::send_mail('it@perfectmobileinc.com', '[PM][Lyca][' . getenv('APP_ENV') . '] Failed to give spiff', $msg);
                }
                ...
                if (!empty($trans->esn)) {
                    ...
                    if (!empty($ret['error_msg'])) {
                        ...
                        Helper::send_mail('it@perfectmobileinc.com', '[PM][Lyca][' . getenv('APP_ENV') . '] Failed to give rebate', $msg);
                    }
                }
