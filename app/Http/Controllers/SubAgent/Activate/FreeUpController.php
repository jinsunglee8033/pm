...
class FreeUpController
{
    public function show(Request $request) {
        ...
        if ($account->act_freeup != 'Y') {
            return redirect('/sub-agent/error')->with([
                'error_msg' => 'Your account is not authorized to do FreeUP Mobile activation. Please contact your distributor'
            ]);
        }

        $ret = Helper::check_parents_product($account->id, 'WFRUPA');
        if($ret != 'Y') {
            return redirect('/sub-agent/error')->with([
                'error_msg' => 'Your account is not authorized to do FreeUP Mobile activation. Please contact your distributor'
            ]);
        ...
        $transactions = $query->where('transaction.account_id', Auth::user()->account_id)
            ->whereIn('product_id', ['WFRUPA', 'WFRUPS'])
        ...
        return view('sub-agent.activate.freeup')->with([
    ...
    public function sim(Request $request, $type) {
        try {
            ...
            if ($type == 'sim') {
                $sim = StockSim::where('sim_serial', $request->code)->whereIn('product', ['WFRUPA', 'WFRUPS'])->first();
                if(empty($sim)){
                    $p = Product::where('id', 'WFRUPA')->first();
                    $sim = StockSim::upload_byos($request->sim, '', 'WFRUPA', $p->carrier, $p->sim_group, $account->id, $account->name);
                }
            }
...
//                }
//
//                $sim = StockSim::where('afcode', $request->code)->whereIn('product', ['WFRUPA', 'WFRUPS'])->first();
//
//                if(empty($sim)){
//                    if(empty($request->sim)){
//                        return response()->json([
//                            'code' => '-2',
//                            'msg' => 'We can not find your activation code. Please Enter exact SIM instead.'
//                        ]);
//                    }
//                }
//            }
...
//            }
            ...
            $mapping = StockMapping::whereIn('product', ['WFRUPA', 'WFRUPS'])->where('sim', $sim->sim_serial)->where('status', 'A')->first();

            $product_id = $sim->product; // == 'ATT' ? 'WFRUPA' : 'WFRUPS';
            ...
            if (empty($sim->amount)) {
                foreach ($denoms as $d) {
                    $v_denom = VendorDenom::where('product_id', $product_id)->where('denom_id', $d->id)->where('status', 'A')->first();
            ...
            $allowed_months = Helper::get_min_month($sim, $account, 'freeup_min_month');
...
//                    'sub_carrier' => 'ATT',
                    ...
                    'sim_charge' => empty($sim->sim_charge) ? 0 : $sim->sim_charge,
                    'sim_rebate' => empty($sim->sim_rebate) ? 0 : $sim->sim_rebate,
                    'sim_consignment_charge' => $sim->type == 'C' ? (empty($sim->charge_amount_r) ? 0 : $sim->charge_amount_r) : 0
                ]
            ]);
    ...
    public function esn(Request $request) {
        try {
            $esn_obj = StockESN::whereIn('product', ['WFRUPA', 'WFRUPS'])->where('esn',$request->esn)->first();
            ...
            $esn_mappings = StockMapping::where('product', $esn_obj->product)->where('esn', $request->esn)->where('status', 'A')->count();
            if ($esn_mappings > 0) {
                $mapping = StockMapping::where('product', $esn_obj->product)->where('esn', $request->esn)->where('sim', $request->sim)->where('status', 'A')->first();
    ...
    public function commission(Request $request) {
        try {
            ...
            $sim_obj = StockSim::whereIn('product', ['WFRUPA', 'WFRUPS'])->where('sim_serial',$request->sim)->first();
            $esn_obj = StockESN::whereIn('product', ['WFRUPA', 'WFRUPS'])->where('esn',$request->esn)->first();

            // Allow BYOD for FreeUP //
            if (empty($esn_obj)) {
                $account = Account::find(Auth::user()->account_id);
                $p = Product::where('id', 'WFRUPA')->first();
                StockESN::upload_byod($request->esn, 'WFRUPA', $p->carrier, $account->id, $account->name);
            }
            ...
            if (!empty($denom)) {
                ...
                $special_spiffs = SpiffSetupSpecial::get_special_spiffs($denom->product_id, $denom->denom, 'S', Auth::user()->account_id, $sim_obj, $esn_obj, $terms);
            }

            $spiff_labels = Array();

            $extra_spiff = StockSim::get_spiff_2_amt('S', $sim_obj);
            if ($sim_obj->type == 'P' && $spiff == 0) {
                if ($spiff == 0) {
                    $sim_label = 'Credit Already Paid' . ($extra_spiff > 0 ? ', Extra Credit $' . $extra_spiff : '');

                    $spiff_labels[] = 'Already Paid';

                } else {
                    $sim_label = 'Credit $' . ($spiff - $extra_spiff) . ($extra_spiff > 0 ? ', Extra Credit $' . $extra_spiff : '');
            ...
            // $esn_label = 'Credit ' . ((!empty($esn_obj) && (($esn_obj->type == 'P' && $rebate == 0) || $esn_obj->status!= 'A')) ? 'Already Paid' : '$' . $rebate);
    ...
    public function get_portin_form($phone_type) {
    ...
    public function post(Request $request) {
        try {
            ...
            $v = Validator::make($request->all(), [
                ...
                'equipment_type' => 'required_if:is_port_in,Y',
                'number_to_port' => 'required_if:is_port_in,Y',
                ...
                'equipment_type.required_if' => 'Equipment type is required',
...
//                $sim_obj = StockSim::whereIn('product', ['WFRUPA', 'WFRUPS'])->where('afcode', $request->afcode)->where('status', 'A')->first();
//
//                if (empty($sim_obj)) {
//                    return response()->json([
//                        'code' => '-2',
...
//                if (empty($request->afcode)) {
                    $sim_obj = StockSim::whereIn('product', ['WFRUPA', 'WFRUPS'])->where('sim_serial', $request->sim)->where('status', 'A')->first();
...
//                }
                ...
                $esn_obj = StockESN::whereIn('product', ['WFRUPA', 'WFRUPS'])->where('esn', $request->imei)->first();
                ...
                if (!empty($esn_obj)) {
                    ...
                    if (strtoupper($sim_obj->sub_carrier) != strtoupper($esn_obj->sub_carrier)) {
                        return response()->json([
                            'code' => '-2',
                            'data' => [
                                'fld'   => 'imei',
                                'msg'   => 'SIM and IMEI are not belong to same carrier'
                            ]
                        ]);
                    }
...
//                    }
            ...
            ### check owner path ###
            if (!empty($sim_obj) && !empty($sim_obj->owner_id)) {
...
//            }
            ...
            ### Special Spiff
            $special_spiffs = SpiffSetupSpecial::get_special_spiffs($denom->product_id, $denom->denom, 'S', $user->account_id, $sim_obj, $esn_obj);
            if (!empty($special_spiffs)) {
                foreach ($special_spiffs as $ss) {
                    $spiff_amt += $ss['spiff'];
                }
...
//            $trans->afcode = $request->afcode;
            ...
            // Helper::log('### FreeUP Activation Post() ###', [
            //     'req' => $request->all(),
            //     'res' => $request->all(),
            //     'info' => $request->all()
            // ]);
            ...
            if ($request->is_port_in == 'Y') {
                ...
                $portinfo->equipment_type = $request->equipment_type;
                $portinfo->number_to_port = $request->number_to_port;
                $portinfo->account_no = $request->account_no;
                $portinfo->account_pin = $request->account_pin;
                $portinfo->first_name = $request->first_name;
                $portinfo->last_name = $request->last_name;
                $portinfo->address1 = $request->address1;
                $portinfo->address2 = $request->address2;
                $portinfo->city = $request->city;
                $portinfo->state = $request->state;
                $portinfo->call_back_phone = $request->call_back_phone;
                $portinfo->email = $request->email;
            }

            $carrier = $trans->product_id == 'WFRUPA' ? 'ATT' : 'SPRINT';
            if ($request->is_port_in != 'Y') {
                // $carrier, $product_id, $product_pin, $activation_code, $iccid, $os_type, $imei, $esn, $zip, $trans_id
                $ret = emida2::freeUp_activation($carrier, $vendor_denom->act_pid, $vendor_denom->pin_pid,
                //$ret = emida::freeUp_activation($carrier, $vendor_denom->act_pid, $vendor_denom->pin_pid,
                    '', $request->sim , $request->handset_os, $request->imei, $request->esn,
                    $request->zip_code, $trans->id);
            } else {
                $ret = emida2::freeUp_portin($carrier, $vendor_denom->act_pid, $vendor_denom->pin_pid,
                //$ret = emida::freeUp_portin($carrier, $vendor_denom->act_pid, $vendor_denom->pin_pid,
                    '', $request->sim , $request->handset_os, $request->imei, $request->esn,
                    $request->zip_code, $trans->id, $portinfo);
            }
            ...
            if ($sim_type == 'C') {
                ...
                if (!empty($ret['error_code'])) {
                    ...
                    Helper::send_mail('it@perfectmobileinc.com', '[PM][FreeUP][' . getenv('APP_ENV') . '] Failed to charge for consignment SIM / ESN', $msg);
                }
            }

            ### Might have no SIM for CDMAV / CDMAS
            if (!empty($sim_obj)) {
            ...
            if (!empty($esn_obj)) {
                if ($esn_obj->status == 'A') {
                    ...
                    $esn_obj->status = $request->is_port_in == 'Y' ? 'P' : 'U';
                    $esn_obj->save();
                }

                if (!empty($sim_obj)) {
                    $mapping = StockMapping::whereIn('product', ['WFRUPA', 'WFRUPS'])->where('esn', $esn_obj->esn)->where('sim', $sim_obj->sim_serial)->where('status', 'A')->first();
                    if (!empty($mapping)) {
                        $mapping->status = 'U';
                        $mapping->update();
                    }
            ...
            if ($request->is_port_in != 'Y') {
                ...
                if ($trans->rtr_month > 1) {
                    if($trans->product_id == 'WFRUPA'){
                        $rtr_product_id = 'WFRUPR';
                    }else{
                    ...
                    if (!empty($error_msg)) {
                        ...
                        $msg .= ' - error : ' . $error_msg;
                        Helper::send_mail('it@perfectmobileinc.com', '[PM][' . getenv('APP_ENV') . '] FREEUP Activation - applyRTR remaining month failed', $msg);
                    }
                }
            }
