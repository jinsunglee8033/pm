...
class AttDocumentsController extends Controller
{

    public function show() {
        //$files = AccountFile::where('account_id', Auth::user()->account_id);
        $types = AccountFileAtt::types();
        $files = [];
        foreach ($types as $k => $v) {
            $file = AccountFileAtt::where('account_id', Auth::user()->account_id)
                ->where('type', $k)
                ->first();
            $o = new \stdClass();
            $o->key = $k;
        ...
        return view('sub-agent.setting.att-documents', [
            'files' => $files
        ]);
    }
