...
class TransactionNewController extends Controller
{
    ...
    public function show(Request $request) {
        try {
            ...
            $query = Transaction::join('product', 'transaction.product_id', 'product.id')
                ...
                ->Leftjoin("spiff_trans as spiff", function($join) {
                    $join->on('transaction.id', 'spiff.trans_id')
                        ->where('spiff.account_type', 'S')
                        ->where('spiff.type', 'S')
                        ->where('spiff.spiff_month', 1);
                })
                ->Leftjoin("commission as commission", function($join) {
                    $join->on('transaction.id', 'commission.trans_id')
                        ->where('commission.account_id', Auth::user()->account_id)
                        ->where('commission.type', 'S');
                })
                ->Leftjoin("promotion as promotion", function($join) {
                    $join->on('transaction.id', 'promotion.trans_id')
                        ->where('promotion.account_id', Auth::user()->account_id);
                });
            ...
            if ($request->excel == 'Y') {
                $transactions = $query->orderByRaw('ifnull(transaction.mdate, transaction.cdate) desc')
                    ->select(
                        'transaction.id',
                        ...
                        'transaction.phone',
                        'transaction.pin',
                        'spiff.spiff_amt as spiff_amt',
                        'spiff.spiff_month as spiff_month',
                        'spiff.special_id as special_id',
                        'commission.comm_amt as comm_amt',
                        'promotion.amount as amount',
                        'promotion.category_id as category_id',
                        'accounts.loc_id',
                        'accounts.outlet_id',
            ...
            $special_id_spiff_amt = $query->sum(\DB::raw("if(spiff.special_id is not NULL, transaction.collection_amt, 0)"));
            $collection_amt = $collection_amt - $special_id_spiff_amt;
            ...
            $special_id_spiff_fee = $query->sum(\DB::raw("if(spiff.special_id is not NULL, transaction.fee + transaction.pm_fee, 0)"));
            $fee = $fee - $special_id_spiff_fee;
            ...
            return view('sub-agent.reports.transaction-new', [
    ...
    public function detail($id) {
        ...
        if (empty($detail)) {
            return redirect('/sub-agent/reports/transaction-new')->withErrors([
        ...
        return view('sub-agent.reports.transaction-new-detail', [
            'detail' => $detail,
            'products' => $products,
...
//                  LycaModifyPortIn($reference_no, $product_id, $sim, $mdn, $account_no, $account_psw, $zip)
            ...
            return redirect('/sub-agent/reports/transaction-new')->with([
                'success' => 'Y'
            ]);
