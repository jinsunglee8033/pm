...
class CommissionController extends Controller
{
    ...
    public function show(Request $request) {
        ...
        return view('admin.reports.vendor.reup.commission', [
            'data' => $data,
        ...
        try {
            ...
            if (Input::hasFile($key) && Input::file($key)->isValid()) {
                ...
                foreach ($results as $row) {
                    ...
                    $rec = ReUPCommission::where('file_name', $file_name)
                        ->where('mdn', $mdn)
                    ...
                    $rec->dealer_residual_payout_org = $dealer_residual_payout;
                    ...
                    if (!empty($trans)) {
                        ...
                        ### 2nd month spiff ###
                        if ($dealer_month2_payout > 0) {
                            $ret_spiff = SpiffProcessor::give_spiff(
                                $trans->account_id,
                                $trans->product_id,
                                $trans->denom,
                                2,
                            ...
                            if (!empty($ret_spiff['error_msg'])) {
                                throw new \Exception($ret_spiff['error_msg'] . ' at Line : ' . $line_no, $ret_spiff['error_code']);
                            }
                        }

                        ### 3rd month spiff ###
                        if ($dealer_month3_payout > 0) {
                            $ret_spiff = SpiffProcessor::give_spiff(
                                $trans->account_id,
                                $trans->product_id,
                                $trans->denom,
                        ...
                        ### give residual ###
                        if ($dealer_residual_payout > 0) {
                            $pay_residual = false;
                            $residual = null;
                            if ($trans->phone_type ==  '3g') {
                                $esn_obj = ROKESN::find($trans->esn);
                                if (!empty($esn_obj)) {
                                    $pay_residual = true;
                                    $residual = $esn_obj->residual_r;
                                }
                            } else {
                                $sim_obj = ROKSim::find($trans->sim);
                                if (!empty($sim_obj)) {
                                    $pay_residual = true;
                                    $residual = $sim_obj->residual_r;
                                }
                            }

                            if ($pay_residual) {
                                if (!empty($residual)) {
                                    $rec->dealer_residual_payout = $plan_value * $residual / 100;
                                    $rec->total_dealer_payout = 
                                        $total_dealer_payout - ($dealer_residual_payout - $rec->dealer_residual_payout);
                                }

                                $rs = new Residual;
                                $rs->type = 'S';
                                $rs->account_id = $account_id;
                                $rs->trans_id = $trans->id;
                                $rs->mdn = $mdn;
                                $rs->act_date = $trans->cdate;
                                $rs->amt = $rec->dealer_residual_payout;
                                $rs->amt_org = $dealer_residual_payout;
                        ...
                        ### give residual to L, M, D ###
                        if ($master_residual_payout > 0) {
                            ...
                            if ($p_account->type == 'D') {
                                ### give residual to distributor ###
                                $rs = new Residual;
                                $rs->type = 'S';
                                $rs->account_id = $p_account->id;
                                $rs->trans_id = $trans->id;
                                $rs->mdn = $mdn;
                                $rs->act_date = $trans->cdate;
                                $rs->amt = $master_residual_payout * $residual_pct / 100;
                                $rs->amt_org = $rs->amt;
                                $rs->comments = 'ReUP RTR Residual';
                                $rs->cdate = Carbon::now();
                                $rs->created_by = $user_id;
                                ...
                                $p_account = Account::find($p_account->parent_id);
                            } else {
                                $residual_pct = 50;
                            }

                            ### give residual to master ###
                            $rs = new Residual;
                            $rs->type = 'S';
                            $rs->account_id = $p_account->id;
                            $rs->trans_id = $trans->id;
                            ...
                            $rs = new Residual;
                            $rs->type = 'S';
                            $rs->account_id = 100000;
                            $rs->trans_id = $trans->id;
                            $rs->mdn = $mdn;
                            $rs->act_date = $trans->cdate;
                            $rs->amt = $master_residual_payout * 50 / 100;
                            $rs->amt_org = $rs->amt;
                            $rs->comments = 'ReUP RTR Residual';
                            $rs->cdate = Carbon::now();
                            $rs->created_by = $user_id;
